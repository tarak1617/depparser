...
def word2vec(fn):
    f = open(fn, 'r')
    wr = open('./Word2Vec/w2vecinput.txt', 'w')
    data = f.read()
    for sent in parse(data):
        for buffer in sent:
            ...
            wr.write(' ')
        wr.write('\n')
    fname = open('./Word2Vec/vecs.bin','w')
    sentences = gensim.models.word2vec.LineSentence('./Word2Vec/w2vecinput.txt')
...
def dictionaries(fn):
    ...
    with open('./dictionaries/deprel.json', 'w') as fp:
        ...
        for bufer in sent:
            ...
            for strin in bufer['feats'].keys():
                if bufer['feats'][strin] not in dictionary:
                    ...
                    i += 1
    with open('./dictionaries/all.json', 'w') as fp:
        json.dump(dictionary, fp)
    return ndeprels
